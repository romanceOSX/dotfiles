...
{ pkgs, ... }:
{
  ...
  # Local split-DNS for the two namespaces alien's system-wide nginx serves
  # (home/system-proxy.nix): *.dev.meddy.com (meddy's dev stack) and
  # *.alien.internal (that host's own infra tools — Portainer, Paisa).
  #
  ...
  # /etc/resolver/<domain> (nameserver 127.0.0.1, port 53) so ONLY those two
  # domains route here — all other DNS keeps using the normal resolvers.
  # `--address=/dev.meddy.com/IP` answers dev.meddy.com and every subdomain
  # under it (web/app/api/swagger), so one line covers a whole vhost set.
  # Coexists with services.tailscale above: different domains, and tailscaled's
  # MagicDNS listens on 100.100.100.100:53, not 127.0.0.1:53 — no port clash.
  # DNS only points the name at the host; TLS is alien's mkcert cert, which
  # must carry a SAN for each domain here (see home/system-proxy.nix).
  #
  # NOTE — dev.meddy.com is a REAL public name, not a reserved-TLD stand-in:
  # this is deliberate split-horizon DNS, buying dev/prod parity for anything
  # that keys off the FQDN (cookie scope, CORS origins, redirect URIs) at the
  # cost of shadowing the real name. Any host running this config can no longer
  # reach a genuinely-deployed dev.meddy.com. Scoped to the `dev.` label only —
  # meddy.com itself and its other subdomains resolve normally.
  services.dnsmasq = {
    enable = true;
    addresses = {
      # alien's Tailscale IP — the box running system-proxy and meddy's compose
      # stack. Stable per node; update only if alien is removed/re-added to the
      # tailnet. Both namespaces are served by the same host, hence same IP.
      "dev.meddy.com" = "100.104.20.52";
      "alien.internal" = "100.104.20.52";
    };
